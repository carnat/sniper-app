---
trigger: auto
description: Sniper Doctrine Core — gate rules, Alpha Filters, DCA Matrix, deployment system
---

# SNIPER DOCTRINE CORE v1.4.5-BETA.1 — EXECUTION LOGIC

> Do not reinterpret, paraphrase, or condense any rule below.

**DOC VERSION:** v1.4.5-BETA.1
...
- **[Mar 16, 2026] v1.4.5-BETA.1:** DC-13 — Satellite Priority Scoring Tier added to Section 6.7 (S1/S2/S3 factors, 30pts max, Watchtower Satellites only, 40% aggregate sleeve limit). DC-13b — Arsenal Satellite Expansion Gate added to Section 6.8 SATELLITE block. DC-13 Core Tie Tiebreaker + split proportion rule added to Section 6.5 Rule 2. Green close single-day confirmation stated explicitly in Section 6.2 Q2. BWXT reclassified Core. Tier B. Commander Mar 16.
- **[Mar 15, 2026] v1.4.4-BETA.1:** Section 6.9 Vault Rebalancing Protocol expanded with full 5-step /vault_review procedure. Section 6.6 Watchtower Graduation Rule — POINTER added to Watchtower Scorecard quarterly protocol. Commander ratified Mar 15.
- **[Mar 15, 2026] v1.4.3-BETA.1:** DC-11 — Zero-Share Factor 2 = 0 Rule added to Section 6.5. Reference prices for zero-share positions prohibited. FN reference price $586.90 deleted. Commander ratified Mar 15.
...
POINTER: State data → portfolio_state (current version).
POINTER: Vault data → vault_registry (current version).
...
### TIER 1 — SHIELD
Thai defensive positions: TISCO, ADVANC, CONDO. Goal: Level 1 (Liquidity). Stop: Yield < 5% OR NPL > 3.5%.
...
POINTER: Vault holdings and overlap → vault_registry (current version).
...
POINTER: Current HWM value → portfolio_state (current version) Section 4.
...
- AXTI is permanently blacklisted under this rule.
- SNDK is permanently blacklisted under this rule (China revenue 27.7%, confirmed Mar 15, 2026).
- AAOI is permanently blacklisted under this rule (China PP&E 45.4% + China-manufactured revenue 44.9%, confirmed Mar 16, 2026).
...
**CURRENT AMMO:** See portfolio_state (current version) Section 4.

**TARGET PRIORITY:**
1. **PRIMARY** — Winner of Section 6.5 DCA Targeting Matrix. Execute at planned size (or per Section 6.2 scaling rules).
2. **REMAINDER** — ASTS fill to target shares (see portfolio_state Section 1 for current fill target — verify before executing). **ZERO-REMAINDER RULE:** If ammo remaining after primary DCA allocation is insufficient to purchase at least 0.1 shares of ASTS, skip the ASTS fill order this cycle. Carry ASTS fill target to next deployment cycle. No partial ASTS purchase below 0.1 shares. Once ASTS reaches fill target, remainder held as USD Cash per USD Cash Holding Rule below.
...
- **ANET:** MSFT or Meta publicly announce primary switching vendor change away from Arista OR publicly shift > 20% of networking spend to internal "white box" custom solutions within a single fiscal year OR AI/cloud vertical drops < 60% of new bookings for 2 consecutive quarters. NOTE: Margin floor permanently breaches < 60% due to aggressive cloud titan pricing leverage also constitutes a thesis break. Microsoft/Meta formally announcing structurally lower AI networking CapEx also constitutes a thesis break. VICTORY PROTOCOL upside review target: $200 (amended Mar 10, 2026 — INVEST-05 ratified. Prior $175 trigger permanently deleted).
...
Eligible pool: Core positions only (VST, VRT, MU, APH, ANET, TSM, FN) PLUS Core Watchtower candidates with /thesis CLEAR (e.g. BWXT, PLTR). Satellites are excluded from matrix scoring.
...
POINTER: Current Matrix scores and winner → portfolio_state (current version) Section 4 Dashboard.
...
Day 0 for all existing NOT SET positions = Mar 13, 2026 (ratification date). No retroactive cascade.
...
Elder dissent on record (Mar 13, 2026): 60-day threshold may create administrative overhead. Revisit threshold at next /council full if operationally disruptive.

**WATCHTOWER GRADUATION RULE**

Candidates are sourced via the Watchtower Scorecard quarterly protocol. See watchtower_scorecard (current version) in Spinoff Space. Next scheduled run: Jun 2026.
...
Authority: Commander + dual-AI ratification (Gemini + Comet).
...
SCOPE: Watchtower Satellites only. Does NOT apply to Core Watchtower candidates or Arsenal Satellites already held (ASTS, ONDS).
...
**ARSENAL SATELLITE EXPANSION GATE (DC-13b — ratified Mar 16, 2026):** Once an Arsenal Satellite reaches its established fill target (e.g. ASTS at 10 shares upon SO-1 resolution), any further capital expansion beyond that fill target requires ALL THREE of the following conditions before execution:
...
**THE SHIELD (TISCO, ADVANC, CONDO):** Goal = Level 1 (Liquidity). Stop = Yield < 5% OR NPLs > 3.5% (TISCO/ADVANC). Condo: net equity tracked separately.
...
POINTER: Vault fund details and E-Class positions → vault_registry (current version).
...
Authority: 8/11 Council, Commander. Tier A — Commander waiver logged (SO-1 operational urgency).
...
**CURRENT APPLICATION:** SO-1 (ASTS +0.7 fill) — submit to broker before ~Apr 11, 2026 blackout. Pre-placed carve-out applies once IO confirms submission timestamp predates Apr 11.
...
POINTER: /review protocol → doctrine_ops (current version) SUB-SECTION 0.4.
...
| Tier | Assets | Value |
|------|--------|-------|
| TIER 1 — HARD CASH | THB Bank Balances and PromptPay accounts. Commander must supply Tier 1 Cash balance at least once per month at /close time. If not updated in > 30 days, all /audit outputs labeled "FLOOR ESTIMATE — Tier 1 cash stale, actual liquid total higher." | 100% |
| TIER 2 — MARKET LIQUID | US Equities, TISCO, ADVANC, Standard Mutual Funds (Non-SSF/RMF/SSFA/SSFE), E-CLASS VAULT FUNDS (SCBKEQTGE, SCBS&P500(E), SCBSEMI(E), SCBGOLDHE) — liquid and switchable | 80% |
| TIER 3 — ILLIQUID / 0 VALUE | SSF wrapper funds, RMF wrapper funds, Condo net equity | 0 — Do not count toward Level 1 target |

CURRENT ESTIMATE METHODOLOGY: (Tier 1 Cash) + (Tier 2 Assets × 0.80).

POINTER: Live E-Class fund values and Tier 2 contributions (×0.80) → vault_registry (current version) Section 4.1 E-Class Liquid Summary.

**LEVEL 2: "THE MUNGER MOMENT"**
Target: $100k USD Total Portfolio. Status: IN PROGRESS (unlocks after Level 1).
...
*END OF DOCTRINE CORE v1.4.5-BETA.1*
